...
import {getAdjustedSizes, getRotatedSizes, getTransforms} from './util';
...
export default {
  ...
  initContainer() {
    const {el, opt, container, cropper} = this;

    cropper.addClass(CLASS_HIDDEN);

    // 恢复原图，获得原图显示情况下，容器的高度、宽度
    $(el).removeClass(CLASS_HIDDEN);
    const containerData = {
      width: Math.max(
        container.offsetWidth,
        Number(opt.minContainerWidth) || 200
      ),
      height: Math.max(
        container.offsetHeight,
        Number(opt.minContainerHeight) || 100
      ),
    };
  ...
  initCanvas() {
    ...
    const naturalWidth = rotated
      ? imageData.naturalHeight
      : imageData.naturalWidth;
    const naturalHeight = rotated
      ? imageData.naturalWidth
      : imageData.naturalHeight;
